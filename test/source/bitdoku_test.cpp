...
TEST_CASE("JSolver", "[library]") {
  auto result = std::array<char, 82>{};
  JSolve("06001000407005000094000000600040000200080001000107260000002"
         "0000690000080080700030",
         result.data(), 2);
  REQUIRE(std::string(result.data()) ==
          "568219374173654928942387156756431892429865713"
          "831972645317528469695143287284796531");
}

TEST_CASE("BENCHMARKING", "[!benchmark]") {
  BENCHMARK("recursive_solve") {
    auto board = Bitdoku{"0600100040700500009400000060004000020008000100010"
                         "72600000020000690000080080700030"};
    board.solve();
...
TEST_CASE("BitdokuGenerator", "[library]") {
  BitdokuGenerator generator;
  auto original_board = generator.get_board();
  generator.generate();
  auto new_board = generator.get_board();
  new_board.solve();
  REQUIRE(original_board == new_board);
}

TEST_CASE("BitdokuGenerator with permutation", "[library]") {
  BitdokuGenerator generator;
  auto original_board = generator.get_board();
  auto original_copy = original_board;
  generator.generate_with_permutation();
  auto new_board = generator.get_board();

  // Both boards should be solvable
  bool original_solvable = original_copy.solve();
  bool puzzle_solvable = new_board.solve();

  REQUIRE(original_solvable == true);
  REQUIRE(puzzle_solvable == true);
}
