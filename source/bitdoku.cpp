...
auto Bitdoku::set_possible(const int board_index) -> bool
{
...
auto Bitdoku::get_possible(const int board_index) -> bit_field
{
    if (bitboards[board_index].is_set()) {
...
auto Bitdoku::solve_parallel(int num_threads) -> bool
{
    ...
    for (int num = 1; num <= 9; ++num) {
        bit_field num_bit = 1 << (num - 1);
        if (is_valid_move(empty_cell, num_bit)) {
            futures.push_back(std::async(std::launch::async, [this, empty_cell,
                                                              num_bit]() {
