...
Cell::Cell(char c) noexcept
    : data(static_cast<bit_field>(1 << (c - '0' - 1)))
{
}

Cell::Cell(bit_field bits) noexcept
    : data(bits)
{
}
...
auto Cell::is_set() const noexcept -> bool
{
  return std::has_single_bit(data);
}

constexpr auto Cell::to_char() const noexcept -> char
{
  if (data == 0 || std::popcount(data) != 1) {
    return '0';
  }
...
const auto Bitdoku::get_board() const
{
  return bitboards;
}
